...
// ✅ SCHRITT 1: ALLE useSignal() VARIABLES ZUERST - KEINE ANDERE REIHENFOLGE!
...
// ✅ SCHRITT 2: STATISCHE DATEN (verwenden keine Signals)
const workflows = [
...
// ✅ SCHRITT 3: ALLE $() FUNCTIONS - JETZT KÖNNEN WIE SIGNALS VERWENDEN
...
order: formFields.value.length + 1, // ✅ formFields ist bereits deklariert!
width: 'full',
required: false,
lightModeVisible: false,
workflowStepBinding: selectedStep.value ? [selectedStep.value] : [], // ✅ selectedStep ist bereits deklariert!
permissions: {
...
formFields.value = [...formFields.value, newField]; // ✅ formFields ist bereits deklariert!
selectedField.value = newField; // ✅ selectedField ist bereits deklariert!


});

const updateField = $((updates: Partial<FormField>) => {
if (!selectedField.value) return; // ✅ selectedField ist bereits deklariert!


const updatedField = { ...selectedField.value, ...updates };
formFields.value = formFields.value.map(f => f.id === selectedField.value?.id ? updatedField : f); // ✅ beide bereits deklariert!
...
formFields.value = formFields.value.filter(f => f.id !== fieldId); // ✅ formFields ist bereits deklariert!
if (selectedField.value?.id === fieldId) { // ✅ selectedField ist bereits deklariert!
selectedField.value = null;
}
});

const moveField = $((fromIndex: number, toIndex: number) => {
const items = [...formFields.value]; // ✅ formFields ist bereits deklariert!
...
formFields.value = items; // ✅ formFields ist bereits deklariert!
...
useTask$(({ track }) => {
track(() => selectedWorkflow.value); // ✅ selectedWorkflow ist bereits deklariert!
...
if (selectedWorkflow.value) {
  ...
  workflowSteps.value = mockSteps; // ✅ workflowSteps ist bereits deklariert!
}


});

// ✅ SCHRITT 5: JSX RETURN - GANZ AM ENDE
return (
...
class={`px-4 py-2 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:rotate-1 ${ lightMode.value  ? 'bg-gradient-to-r from-yellow-400 to-orange-400 text-white shadow-lg shadow-yellow-400/30 hover:shadow-xl hover:shadow-yellow-400/40'  : 'bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 hover:from-gray-200 hover:to-gray-300 border border-gray-300 hover:border-gray-400' }`}
>
⚡ Light Mode
</button>
<button
onClick$={() => previewMode.value = !previewMode.value}
class={`px-4 py-2 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:-rotate-1 ${ previewMode.value  ? 'bg-gradient-to-r from-green-400 to-emerald-400 text-white shadow-lg shadow-green-400/30 hover:shadow-xl hover:shadow-green-400/40'  : 'bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700 hover:from-gray-200 hover:to-gray-300 border border-gray-300 hover:border-gray-400' }`}
>
👁️ Preview
</button>
<button class="px-6 py-3 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-white rounded-xl font-bold hover:from-blue-600 hover:via-purple-600 hover:to-pink-600 transition-all duration-300 transform hover:scale-105 hover:shadow-xl shadow-lg shadow-purple-500/30 hover:shadow-purple-500/40">
💾 Save Form
</button>
</div>
</div>
